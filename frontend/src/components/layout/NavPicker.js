            ...
            <div>
              ...
              <small>
                {items.length > 0 &&
                  ...
                  items.find((item) => item.id == current).name}
              </small>
            </div>
          </div>
        </div>
        <i
          onClick={() => setNavOpen(`${navOpen != name ? name : ""}`)}
        ...
        {items.map((item) => (
          <div
            ...
            onClick={() =>
              current != item.id
                ? (toast.success(
                    `You have switch to ${name.slice(0, -1).toLowerCase()} '${
                      item.name
                    }'`
                  ),
          ...
          >
            <div className={`sidenav__link no-link`}>
              <div className="sidenav__link-text">{item.name}</div>
            </div>
          </div>
        ))}
      </div>
