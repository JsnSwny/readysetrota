...
const SideNav = ({ sidebarOpen, setSidebarOpen, confirmProps }) => {
  ...
  useEffect(() => {
    if (user) {
      user.business
        ? setUserName(user.business.name)
              ...
              >
                <NavLink
                  ...
                  dropdownAction={() =>
                    setNavOpen(
                      `${
                        navOpen != "staff-management" ? "staff-management" : ""
                      }`
                    )
                  }
                    ...
                    <div className={`sidenav__links-container`}>
                      <div
                        onClick={() => {
                          setConfirmOpen(true);
                          setMessage(
                            "Are you sure you want to cancel your premium subscription? (Your premium will continue until)"
                          );
                          setOnConfirm(() => () => {
                            setConfirmOpen(false);
                            dispatch(
                              cancelSubscription(user.profile.stripe_id)
                            );
                            toast.success(
