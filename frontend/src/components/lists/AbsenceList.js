...
const AbsenceList = ({listProps}) => {
    ...
    return (
        <table>
                <TableHeaders check={false} items={shifts} titles={[{name: 'Date'}, {name: 'Start Time'}, {name: 'End Time'}, {name: 'Absence'}, {name: 'Length', nomobile: true}, {name: 'Employee'}]} selectAll={selectAll} setSelectAll={setSelectAll} setSelected={setSelected} />
            {shifts.map(item => (
