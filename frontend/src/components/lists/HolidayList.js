...
const HolidayList = ({ listProps }) => {
  ...
  const approveAction = {
    name: "Approve",
    action: () => action("Holiday Approved", batchApprove(selected, true)),
  };
  const unapproveAction = {
    name: "Unapprove",
    action: () => action("Holiday Unapproved", batchApprove(selected, false)),
  };

  const allFilter = {
    name: "All",
    action: () => filter("Showing all holidays", setFilterHolidays(holidays)),
  };
  const approvedFilter = {
    ...
    action: () =>
      filter(
        ...
        setFilterHolidays(holidays.filter((item) => item.approved))
      ),
  };
  const unapprovedFilter = {
    ...
    action: () =>
      filter(
        "Filtering by unapproved",
        setFilterHolidays(holidays.filter((item) => item.approved == false))
      ),
  };
  const unmarkedFilter = {
    name: "Unmarked",
    action: () =>
      filter(
        "Filtering by unmarked",
        setFilterHolidays(holidays.filter((item) => item.approved == null))
      ),
  };
  ...
  return (
    <Fragment>
      ...
      <DropButton
        title="Filters"
        actions={[allFilter, approvedFilter, unapprovedFilter, unmarkedFilter]}
      />
      <table>
        <TableHeaders
          check={true}
          items={filterHolidays}
          titles={[
            { name: "Employee" },
            { name: "Type" },
            { name: "Date" },
