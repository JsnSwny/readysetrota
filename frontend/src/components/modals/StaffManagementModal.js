...
import { addDepartment, updateDepartment, updatePosition, addPosition, updateSite, addSite, deleteSite, deleteDepartment, deletePosition } from "../../actions/employees";
...
const StaffManagementModal = (props) => {
    ...
    return (
        <div className="form">
            <div className="form__image"><i class="fas fa-users-cog"></i></div>
            <p className="form__subheading">Staff Management - {form}</p>
            <h1 className="form__heading">
                {name ? name : !update ? title : name}
            </h1>
            <form onSubmit={onSubmit} className="form__form">
                ...
                {errors.name && errors.name != true && <p className="error">{errors.name}</p>}
