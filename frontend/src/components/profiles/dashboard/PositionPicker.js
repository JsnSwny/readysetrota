...
const PositionPicker = (props) => {
  ...
  let positionsEqual = positions.some((item) => item.order == null)
    ? false
    : newPositions.map((item) => item.order).toString() ==
      positions.map((item) => item.order).toString();

  useEffect(() => {
    setNewPositions(
      positions.sort(
        (a, b) => (a.order === null) - (b.order === null) || a.order - b.order
      )
    );
  }, [positions]);
