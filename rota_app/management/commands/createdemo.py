...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while not user:
            ...
            business = Business(owner=user, name='Demo Account', plan='P', total_employees=20)
            business.save()
            profile = UserProfile(user=user, role='business', stripe_id='0')
            site = Site(business=business, name="Site 1")
            site.save()

            department1 = Department(name="Department 1", site=site, owner=user, business=business)
            department1.save()
            department2 = Department(name="Department 2", site=site, owner=user, business=business)
            department2.save()

            position1 = Position(name="Position 1", department=department1, owner=user, business=business)
            position1.save()

            position2 = Position(name="Position 2", department=department2, owner=user, business=business)
            position2.save()

            position3 = Position(name="Position 3", department=department2, owner=user, business=business)
            position3.save()

            position4 = Position(name="Position 4", department=department1, owner=user, business=business)
            position4.save()
            ...
            for i in range(1, 11):
                ...
                employee = Employee(first_name='Staff', last_name=str(i), business=business, owner=user, wage=0.00, default_availability=default_availability)
